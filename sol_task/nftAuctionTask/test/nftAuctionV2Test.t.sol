...
import {NftAuctionV2Deploy} from "../script/nftAuctionV2Deploy.s.sol";

contract nftAuctionV2Test is Test {
    nftAuctionV2 private v2Auction;
    ...
    uint256 private constant NEW_STYLE = 2; // 1: v1拍卖合约, 2：Factory生成v1合约，, 3: Deploy 获取Factory ，然后创建v2, 4: Deploy V1, 然后update v2

    function setUp() public {
        // Setup code if needed
        if (NEW_STYLE == 1){
            v2Auction = nftAuctionV2(newNftAuctionV2());
            v2Auction.initialize(USER1, FEE_ACCOUNT, FEE_PERCENT);
        }else if (NEW_STYLE == 2){
            v2Auction = nftAuctionV2(newNftAuctionFactory());
        }else if (NEW_STYLE == 3){
            v2Auction = nftAuctionV2(newNftAuctionV2Deploy());
        }
    ...
    function newNftAuctionV2() public returns(address) {
        vm.startBroadcast();
    ...
    function newNftAuctionFactory() public returns(address) {
        ...
        nftAuctionV2 template = new nftAuctionV2();
    ...
    function newNftAuctionV2Deploy() public returns(address) {
        vm.startBroadcast();
        NftAuctionV2Deploy v2d = new NftAuctionV2Deploy();
        vm.stopBroadcast();

        nftAuctionFactory nftAuctionFacory = nftAuctionFactory(v2d.DeployAuctionV2());
        ...
        console.log("newNftAuctionV2Deploy auction 1 version is:", version);
    ...
    function testv2AuctionVersion() public view{
        assertEq("v2.0.0", v2Auction.getVersion(), "nftAuction version is v1.0.0.");
    }

    function testInitializev2Auction() public view {
        address auctionOwner = v2Auction.owner();
        assertEq(auctionOwner, getAdmin(), "v2Auction owner is USER1.");

        address feeAccount = v2Auction.feeAccount();
        assertEq(feeAccount, getFeeAccount(), "v2Auction feeAccount is FEE_ACCOUNT_2");

        uint256 feePercent = v2Auction.feePercent();
        assertEq(feePercent, getFeePercent(), "v2Auction feePercent is FEE_PERCENT_2");
    }

    function testV1SetPriceFeedAndGetPriceUSD() public {
        // 设置ETH和ET1的喂价器预言机
        vm.startPrank(getAdmin());
        v2Auction.setPriceFeed(address(0), user1.mETHtoUSDPriceFeed);
        v2Auction.setPriceFeed(user1.eT1Token,user1.mET1toUSDPriceFeed);
        vm.stopPrank();

        // 计算ETH价格是否正确
        uint256 ethPrice = v2Auction.getPriceUSD(address(0), 1);
        console.log("ETH to USD priceFeed is: ", hc.ANVIL_PRICEFEED_ETH_USD());
        assertEq(ethPrice, 200, "ETH to USD priceFeed is wrong");

        // 计算ET1价格是否正确
        uint256 et1Price = v2Auction.getPriceUSD(user2.eT1Token, 100);
        console.log("ETH to USD priceFeed is: ", hc.ANVIL_PRICEFEED_ETH_USD());
    ...
    modifier auctionPrepare() {
        ...
        IERC721(user3.nftAddress).approve(address(v2Auction), user3.nftTokenId);
        ...
        vm.prank(user3.userAddress);
        uint256 auctionId = v2Auction.createAuction(
            user3.nftAddress, 
            user3.nftTokenId, 
            1, startTime, endTime);
    ...
    function testv2AuctionCreateStatus() public auctionPrepare{
        address nftOwner = IERC721(user3.nftAddress).ownerOf(user3.nftTokenId);
        assertEq(nftOwner, address(v2Auction), "IERC721 nft has transfer Auction contract.");
    }
    ...
    function testv2AuctionRevertBidBeforeStart() public auctionPrepare{
        ...
        v2Auction.placeBidETH{value: 0.1 ether}(user1.nftTokenId);
    }

    function testv2AuctionUser1BidETHCheckStatus() public auctionPrepare{
        ...
        vm.prank(user1.userAddress);
        v2Auction.placeBidETH{value: 0.1 ether}(auctionId);

        user1Balance = payable(user1.userAddress).balance;
        console.log("user1Balance after bid: ", user1Balance);

        (address bidder,address bidToken, uint256 bidAmount,uint256 bidUsd) = v2Auction.getHighestBidInfo(auctionId);
    ...
    function testv2AuctionBidSendBackETH() public auctionPrepare{
        vm.warp(block.timestamp + 30);
        ...
        v2Auction.placeBidETH{value: 0.2 ether}(auctionId);
    ...
    function testv2AuctionUser1BidERC20CheckStatus() public auctionPrepare{
        vm.warp(block.timestamp + 30);
        ...
        vm.prank(user1.userAddress);
        IERC20(user1.eT1Token).approve(address(v2Auction), user1_bidprice_ERC20);

        vm.prank(user1.userAddress);
        v2Auction.placeBidERC20(auctionId, user1.eT1Token, user1_bidprice_ERC20);
    ...
    function testv2AuctionBidSendBackERC20() public auctionPrepare{
        vm.warp(block.timestamp + 30);
        ...
        uint256 user1_bidprice_ERC20 = 100 * 10 **18;
        vm.prank(user1.userAddress);
        IERC20(eT1Token).approve(address(v2Auction), user1_bidprice_ERC20);

        vm.prank(user1.userAddress);
        v2Auction.placeBidERC20(auctionId, eT1Token, user1_bidprice_ERC20);
        ...
        vm.prank(user2.userAddress);
        IERC20(eT1Token).approve(address(v2Auction), user2_bidprice_ERC20);

        vm.prank(user2.userAddress);
        v2Auction.placeBidERC20(auctionId, eT1Token, user2_bidprice_ERC20);
    ...
    function testv2AuctionUser1BidEthUser2BidERC20() public auctionPrepare{
        vm.warp(block.timestamp + 30);
        uint256 auctionId = 1;
        address eT1Token = user1.eT1Token;
    ...
    function testv2AuctionUser1BidERC20User2BidEth() public auctionPrepare{
        vm.warp(block.timestamp + 30);
        uint256 auctionId = 1;
        address eT1Token = user1.eT1Token;
        
        uint256 user1_bidprice_ERC20 = 100 * 10 **18; // 10 USD
        uint256 user1_bidprice_ERC20_USD = v2Auction.getPriceUSD(eT1Token, user1_bidprice_ERC20);
        console.log("user1_bidprice_ERC20_USD : ", user1_bidprice_ERC20_USD);

        uint256 user2_bidprice_ETH = 1 * 10 **17; // 20 USD
        uint256 user2_bidprice_ETH_USD = v2Auction.getPriceUSD(address(0), user2_bidprice_ETH);
        ...
        // user2出价
        vm.prank(user2.userAddress);
        v2Auction.placeBidETH{value: user2_bidprice_ETH * 1 wei}(auctionId);
    ...
    function testv2AuctionUser1BidOutTime() public auctionPrepare {
    ...
    function testv2AuctionUser1BidETHEndAuction() public auctionPrepare {
        vm.warp(block.timestamp + 30);
        // user1出价
        uint256 auctionId = 1;
        uint256 user1Balance = payable(user1.userAddress).balance;
        ...
        */
        ...
        vm.prank(user3.userAddress);
        v2Auction.endAuction(auctionId);
    ...
    function testv2AuctionUser1BidErc20EndAuction() public auctionPrepare {
        vm.warp(block.timestamp + 30);
        // user1出价
        uint256 auctionId = 1;
        address eT1Token = user1.eT1Token;
